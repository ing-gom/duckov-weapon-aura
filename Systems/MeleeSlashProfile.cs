...
namespace WeaponAura.Systems
{
    ...
    /// </summary>
    [Serializable]
    public class MeleeSlashProfile : ISerializationCallbackReceiver, ILayerHost
    {
        public string name = "";
    ...
    public static class MeleeSlashProfiles
    {
        ...
        /// </summary>
        private static MeleeSlashProfile[] Read(ProfileSetData? data)
        {
            if (data == null)
                return Array.Empty<MeleeSlashProfile>();

            var unpacked = ProfileJson.Unpack<MeleeSlashProfile>(data.items);
            if (unpacked.Length > 0)
                return unpacked;

            return data.grades ?? Array.Empty<MeleeSlashProfile>();
        }
        ...
        /// </summary>
        public static MeleeSlashProfile? Resolve(int quality, int weaponTypeId)
        {
            var over = WeaponOverrides.Resolve(weaponTypeId);
            return over != null ? over.melee : Resolve(quality);
        }
        ...
        public static bool Save(out string path)
        {
            ...
            try
            {
                ...
                UnityEngine.Debug.Log(
                    $"[WeaponAura] 저장 직렬화(참격): {json.Length}자, 항목 {Runtime.Length}개, " +
        ...
        public static bool Restore(string json)
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"[WeaponAura] 근접 참격 되돌리기 실패: {ex.Message}");
                return false;
            }
        }
